...
INSTRUCTIONS = """\
To write an AWS Step Functions state machine, write it in Python with `sfnx` instead of writing Amazon States Language by hand. Mark the entry function with `@state_machine` (`from sfnx import aws, state_machine`): its parameter is the execution input and its return value the output. Call AWS with `aws.sdk.<service>.<operation>(...)` for an SDK integration or `aws.optimized.<service>.<operation>(...)` for an optimized one, the service as its integration ARN names it, the operation in snake_case and the API parameters in PascalCase, such as `aws.optimized.lambda_.invoke(FunctionName="charge", Payload=input)`, and use `if`, `for`, `while` and `try` for the flow. Annotate a value where its type decides the operator: `total: float = input["total"]`. Write a transform sfnx has no spelling for as `jsonata("<expression>", name=value)`. After each edit run `sfnx compile app.py`: exit 0 prints the definition on stdout, and exit 1 prints `app.py:<line>:<column>: <problem>; <what to write instead>` on stderr. A file with several state machines needs `sfnx compile app.py -o out/`, which writes `out/<function>.asl.json`.\
"""
