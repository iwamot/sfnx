...
A state is named after what it does: the variable it assigns, `return`, `raise`, `if`, `for`, `while`, `wait`, `map`, `parallel`, the action of a Task on its own line (`getItem`, `invoke`), or `next` for a Pass that only moves a `for` loop on to its next item, whose counter already names the Pass that starts the loop. Repeated names get serials (`amount`, `amount_2`). Paths that end the same way, with the same value returned or the same error raised with the same message, share one Succeed or Fail, as a hand-writer ends them at one state, and paths whose last states do the same, such as the same Task call before the same end, share those states too, as a hand-writer runs a clean-up the paths share once. A state that reads the context's `State`, which names the state it is read in, keeps its own. States inside a Parallel branch or a Map are prefixed with the function that holds them (`email.return`).

States split only where ASL requires it: a Task, a Choice, an assignment of a name assigned just before it, and one that reads a value that changes on evaluation (the time, a random value, a `jsonata()` expression that is not settled, as [JSONata expressions](#jsonata-expressions) describes) assigned just before it. A name assigned again shares the state when the new value reads the old one in a place always evaluated, such as the test of `x = 0 if not isinstance(x, (int, float)) else x`, and the old value cannot be missing (a value written in the source, a variable, or `d.get()`). An old value that can neither fail nor be missing, such as one written in the source, is replaced in the same state wherever the new value reads it, as there is nothing to evaluate. An `if` whose branches (and `elif` and `else`) each only assign the same variable is compiled as that variable's conditional expression, without a Choice, unless a branch assigns a value written in the source to a variable the function compares elsewhere with `==`, `!=`, `is` or `is not`. Other assignments share one Pass, one reading what another assigns reading that assignment's expression in its place, or go in the `Assign` of what is the only way to them: a Wait right before them, the Choice rule of the `if` branch or the `while` or `for` body they start, or the Choice's own `Assign`, which only its `Default` applies, for `else`, for what follows an `if` whose branches all end (in `return`, `raise`, `break` or `continue`), and for what follows a loop left without `break`. Where paths join, or where the one path there comes from a Choice rule, a Choice's `Default` or a catcher (after a function called directly that returns from a loop, or after an `except` clause), the assignments that follow go in the `Assign` of the last state or Choice rule on each path instead, when every one of them can take them: a Pass, a Wait, a Choice rule or a catcher, a Choice for its `Default`, or a Task, a Parallel or a Map as above, whose `Assign` does not assign a name they read or assign. At the start of an `except` clause, the catchers assign the error, and the assignments read it as the error output they assign, `$states.errorOutput`, so they go in the catchers' `Assign` too. An `if` right after an `if` whose branches end is a rule of the same Choice, after the first's rules; what that `Default` assigns goes in each of its rules too. More generally, where a Choice rule or a `Default` leads straight to another Choice, such as an `if` that starts the body of an `if` or a loop, or the test of a loop right after an `if`, the first Choice takes the second's tests in its place: a rule `a` becomes `a and b` for each test `b` of the second, in its order, then `a` alone for the second's `Default`. The tests read what the rule or `Default` assigns as the expressions it assigns them, bound once in a block where a test reads one more than once, unless one gives another value each time it is evaluated (the time, a random value, or a `jsonata()` expression that reads one). What the rule or `Default` assigns goes in each rule that takes its place, so the definition repeats those assignments, and the second Choice stays for the other ways into it. A Choice that leads back to itself through Choices alone, as a `while` loop with nothing but assignments in its body does, is taken in once into each Choice. Where every path to a Choice has given a variable a value written in the source, such as a flag an `except` clause sets (`failed_at = "payment"`) and the others leave `None`, a test of it (`is None`, `is not None`, `==` or `!=` a value written in the source, or `<`, `<=`, `>` or `>=` a number written in the source when the value is a number) is decided on each path, and the path goes where the test would send it, as a hand-writer's Catch leads straight to its compensation; a Choice no path reaches any more goes. The assignments of the rule or the `Default` it would take go in the `Assign` of the transition that now skips the Choice, when that transition is a Choice rule, a catcher, a Pass, a Wait, a Choice's `Default`, or a Task, a Parallel or a Map as above, and they read no name it assigns; otherwise the path keeps the Choice. A Task's result goes in the Task's `Assign`, and a machine that returns a Task's result ends on that Task. A `return` right after a Wait is the Wait's `Output`, which Step Functions evaluates when the wait is over, so the Wait ends the machine or the branch; the assignments between them go in the `Output` too, as for a return after assignments. A `return` right after assignments no Task holds writes them into its `Output`, one Succeed without a Pass, when each cannot fail (as above) or the `return` is that variable, and none changes on evaluation. Assignments right after a Task, a Parallel or a Map go in that state's `Assign`, and so do the step of a `for` loop whose body ends with one and the start of a `for` loop right after one, when it reads nothing that state assigns, reading the result it assigned as `$states.result`, and a `return` right after them, or right after the state, is its `Output`, so the state ends the machine or the branch. Inside a `try`, they go in the `Assign` and `Output` of a Task, a Parallel or a Map too when they are in the same `try` body as the Task, where its Catch takes their failure as the `except` does, unless a way on from the Task's catchers reads a variable the Task or they assign before one of them that can fail or be missing: a failing `Assign` assigns none of them, the Task's result included, where Python keeps what was assigned before the failing statement. A variable assigned only after every statement that can fail holds, on the way from the catchers, the value from before the Task in both. Reading variables and keys with `d.get()`, testing them with `isinstance()`, `is None`, `==`, `!=`, `in`, `not`, `and` and `or`, choosing with a conditional expression, and writing lists and dicts of such values cannot fail, so such statements go in the Task after the `try` as well. They keep a Pass or a Succeed of their own then, after a state that retries on `Exception` or `QueryEvaluationError`, when one of them reads the context's `State` or a `jsonata()` expression that is not settled, or the time or a random value after a Parallel or a Map, and when an assignment unpacks (`a, b = ...`). A `return` of a value written in the source, `None` included, cannot fail, so it is the `Output` even inside a `try` or after such a retrier, and where paths join before it, as after a `try` whose clause passes or an `if` whose branch calls a Task, a Task, a Parallel or a Map that leads to it ends with it as its `Output`, and the Succeed stays only for the other paths. Assignments of values written in the source cannot fail either, so they go in the `Assign` of such a state too, right after it or where paths join after it. The time and a random value go in the `Assign` of a Task or a Wait, which Step Functions evaluates when the state ends, where Python reads them after it. What reads one of them more than once reads the variable in a state after it, as each reading of the expression would give another value. The assignments that start the machine, a branch or a map's function, where nothing before them can take them, go in the first state when it is a Choice, whose rules and own `Assign` hold them on every path, or a Task that could take the assignments after it, or a Wait, a `parallel()` or a map when none of them can fail: the state reads each as its expression, and its `Assign` assigns it. A Choice right after them that they decide, as `if x is None` right after `x = None`, is skipped first, and a `return` that then follows them directly writes them into its `Output` when none can fail.
...
| `24 * 60 * 60`, `7 // 2`, `0 < 1`, `"a" + "b"`, `[1] + [2]`, `-x` where the operands are integers, strings or lists written in the source | the value, `86400`, `3`, `true`, `"ab"`, `[1, 2]`, as a hand-writer writes it; of the comparisons of strings, only `==` and `!=` are, as JSONata orders strings by UTF-16 units; an integer past 2^53, which a double does not hold, stays an expression |
...
- The API parameters are keyword arguments in PascalCase, and are the `Arguments`. They are spelled as Step Functions spells them, at every level of the `Arguments` and in the result: an SDK integration takes the member names of the AWS SDK for Java v2, which write the capitals a name starts with in lowercase after the first, all but the last when a lowercase letter follows (`DBInstanceIdentifier` is `DbInstanceIdentifier`, `ACL` is `Acl`, DynamoDB's `BOOL` is `Bool`, and `MultiAZ` stays `MultiAZ`), and an optimized integration botocore's with the first letter capitalized (`BOOL` stays `BOOL`). A dict unpacked with `**` is merged into them, and one unpacked on its own is the `Arguments` as it is. The Task's settings are lowercase: `timeout=` (`TimeoutSeconds`), `heartbeat=` (`HeartbeatSeconds`), `role=` (`Credentials.RoleArn`), `retry=`, and `pattern=` (`".sync"`, `".sync:2"` or `".waitForTaskToken"`, which ends the ARN; SDK integrations take only `".waitForTaskToken"`).
...
- SDK integrations are checked against botocore: the service, the operation, argument names, the keys of the dicts written in them, and the required arguments. A name botocore spells otherwise, such as `DBInstanceIdentifier`, is rejected with the spelling to write. Service names follow the AWS SDK for Java (`sfn`, `eventbridge`, `cloudwatchlogs`); botocore's names that differ (`logs`) are rejected with the name to write. Whether Step Functions supports a service or action that botocore has is not checked, nor are the types of argument values; ValidateStateMachineDefinition rejects a service or action Step Functions does not support, such as `aws.sdk.sts.assume_role`, and checks the types it can, such as a number written as a Lambda `Payload`.
- Optimized integrations are checked for argument names, the keys of the dicts written in them, and required arguments when botocore has the action, and which pattern an action supports is not checked. HTTP Tasks need `ApiEndpoint`, `Method` and a connection. Activity and Lambda function ARNs, and ARNs containing `${...}`, are passed as written.
...
- **sfnx exports** the Step Functions errors a Retry or Catch can name: `Timeout`, `TaskFailed`, `Permissions`, `HeartbeatTimeout`, `DataLimitExceeded`, `ExceedToleratedFailureThreshold`, `ItemReaderFailed`, `ResultWriterFailed`, `QueryEvaluationError`, `HttpSocket`. `except Exception` is `States.ALL`, which does not catch `States.Runtime` and, according to the Step Functions documentation, `States.DataLimitExceeded`. `States.BranchFailed` is not among them: a `parallel()` whose branch raises fails with the raised error, which an `except` of that class catches.
...
- **`raise Declined("message")`** is a Fail with that `Error` and `Cause`, unless an `except` around it catches it, as below. Assignments right before it that can neither fail nor be missing have no Pass, as nothing reads them after the Fail; the message reads them as their expressions. A message known not to be a string goes through `$string()`, or is written as its text where the value is known while the file compiles. Python's built-in exceptions and the Step Functions errors cannot be raised, and `from ...` is left out, as a Fail has no chained cause. A Fail always has an `Error`; the Fail without one that ASL allows has no spelling.
...
- Pass, Choice and Wait states cannot catch. An expression that fails in a statement of the body without a `task()`, `parallel()` or map call is caught when the statement goes in the `Assign` or the `Output` of the Task before it, as above, and is not caught otherwise ([The ASL's own semantics](#the-asls-own-semantics)). A `raise` in the body that one of its `except` clauses catches goes to that clause, the innermost `try` first and its clauses in order, as in Python, instead of a Fail: `as e` holds `{"Error": ..., "Cause": ...}` as a Catch assigns it, with the message as its text, and the assignment goes in the Choice rule or the state before the `raise` where it can, as other assignments do. A `try` whose body has no Task, Parallel, Map or such `raise` is rejected, and so is a bare `raise` in a clause that an `except` around it would catch: a Fail ends the execution. `finally` and a bare `except:` are rejected.
...
- An expression that reads only the values the call binds, and names it binds itself that no variable has, and calls nothing that changes (`$random`, `$uuid`, `$now`, `$millis` or `$eval`, under their names or bound to another), is settled, a value like any other: its assignment goes in the state before or after it as a Python expression's would. Any other is read once wherever the generated JSONata would otherwise write it twice, as `jsonata(...) % 2` does: it is bound at the start of a block, a list a `for` iterates is saved before the loop, and its assignment keeps a Pass of its own. A `$` in a string or a regular expression is taken for a name, which keeps the expression among the latter.
