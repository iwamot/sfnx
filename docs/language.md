...
- **`raise Declined("message")`** is a Fail with that `Error` and `Cause`, unless an `except` around it catches it, as below. A message known not to be a string goes through `$string()`, or is written as its text where the value is known while the file compiles. Python's built-in exceptions and the Step Functions errors cannot be raised, and `from ...` is left out, as a Fail has no chained cause. A Fail always has an `Error`; the Fail without one that ASL allows has no spelling.
