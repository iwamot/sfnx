...
- **Expressions** are Python operators, conditional expressions, list and dict comprehensions, f-strings (with a width or a number's digits as the format spec), slices and dicts with `**`, and the functions and methods JSONata has a counterpart for:
  ...
  - a datetime from `datetime.now()`, `datetime.fromisoformat(text)` or `datetime.fromtimestamp(seconds)`, moved by a `timedelta` written in the source (`datetime.now() + timedelta(hours=1)`) and converted where it is made: `str()` or an f-string for the timestamp text, `.timestamp()` for the seconds, `.strftime("%Y-%m-%d")` for the text a picture string writes, `wait(until=...)` for the moment to wait for, and `(dt - dt2).total_seconds()` for the seconds between two of them
