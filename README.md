...
- **Expressions** are Python operators, `len`, `float`, `int`, `str`, `bool`, `isinstance`, `json.loads`, `str(uuid.uuid4())`, the string methods `split`, `replace`, `lower`, `upper` and `join`, dicts with `**`, conditional expressions, list comprehensions and f-strings.
...
- **Anything else** (`with`, other methods, slices, `lambda`, ...) is rejected with what to write instead; [the reference](https://github.com/iwamot/sfnx/blob/main/docs/language.md#what-is-rejected) lists it.
