...
- **Expressions** are Python operators, `len`, `float`, `int`, `str`, `bool`, `isinstance`, `json.loads`, `str(uuid.uuid4())`, the string methods `split`, `replace`, `lower`, `upper` and `join`, slices, dicts with `**`, conditional expressions, list comprehensions and f-strings.
